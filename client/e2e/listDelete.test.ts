...
test.describe.serial('Delete a list from a board', () => {
  const user = fakeUser()
  const board = fakeBoard()
  const list = fakeList()

  test('user can delete a list from a board', async ({ page }) => {
    await asUser(page, user, async () => {
      ...
      const dropdownToggle = page
        .locator('div.fixed-spacing')
        .locator('button[data-testid="dropdown-toggle"]', { hasText: '' })
      await expect(dropdownToggle).toBeVisible()
      await dropdownToggle.click()

      const deleteButton = page.locator('button.text-red-600', { hasText: 'Delete list' })
      ...
      await expect(
        deleteDialog.getByText(`Are you sure you want to delete the list "${list.title}"?`)
      ).toBeVisible()
      ...
      const listContainer = page.locator('div.fixed-spacing')
      await expect(listContainer.locator('h2', { hasText: list.title })).not.toBeVisible()
    })
  })
})
